...
using EntglDb.Legacy.Metadata;
using FluentAssertions;
using Xunit;

namespace EntglDb.Legacy.Tests;
...
public class PeerCollectionTests
{
    ...
    private class InMemoryPeerStore : IPeerStore
    {
        ...
        public Task<IEnumerable<OplogEntry>> GetOplogAfterAsync(HlcTimestamp since, IEnumerable<string>? collections = null, CancellationToken cancellationToken = default)
        {
            var query = _oplog.Where(e => e.Timestamp.CompareTo(since) > 0);
        ...
        {
            var query = _oplog.Where(e => e.Timestamp.NodeId == nodeId && e.Timestamp.CompareTo(since) > 0);
            if (collections != null && collections.Any())
            {
                query = query.Where(e => collections.Contains(e.Collection));
            }
            return Task.FromResult<IEnumerable<OplogEntry>>([.. query.OrderBy(e => e.Timestamp)]);
        }
